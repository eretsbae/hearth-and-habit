...
2. **비즈 앱 전환 (필수)** — 현행 카카오 정책상 카카오톡 메시지 API("나에게 보내기" 포함)는
   비즈 앱에서만 쓸 수 있습니다. 무료이고, 사업자등록번호가 없어도 됩니다:
   - **사업자번호가 있는 경우**: 앱 선택 → **앱 설정 → 일반** → 비즈 앱 전환에서 바로 전환
   - **개인(사업자번호 없음)**: [카카오 데브톡](https://devtalk.kakao.com) 로그인 →
     **추가 기능 신청** 카테고리에 글 작성 — 제목 `[개인 개발자 비즈 앱 전환 신청]`,
     본문에 **앱 ID**(앱 설정 → 일반에서 확인되는 숫자)와 용도(예: "개인 블로그 주간 통계를
     카카오톡 나에게 보내기로 수신") 기재. 보통 영업일 1일 내 승인됩니다.
   - 승인 전까지 아래 단계의 talk_message 동의항목이 잠겨 있을 수 있습니다 — 승인 후 진행하세요.

3. 앱 선택 → **앱 설정 → 플랫폼 → Web 플랫폼 등록**
   - 사이트 도메인: `http://localhost:8765`
4. **제품 설정 → 카카오 로그인** → 활성화 **ON**
   - **Redirect URI** 등록: `http://localhost:8765`
5. **제품 설정 → 카카오 로그인 → 동의항목** → **카카오톡 메시지 전송(talk_message)** → 사용 설정
...
| talk_message 동의항목이 잠겨 있음 / 메시지 API 사용 불가 안내 | 비즈 앱 전환이 안 된 상태입니다 — 위 1단계의 "비즈 앱 전환" 참고 (개인은 데브톡 신청) |
