...
- **Pinterest 게시**: 글마다 세로형 핀 이미지(1000×1500)를 자동 생성합니다. 검색 색인이 아직 안 잡힌 신규 사이트가 실제 방문자와 외부 링크를 확보하는 가장 빠른 경로입니다. 설정: **[docs/PINTEREST_SETUP.md](PINTEREST_SETUP.md)**
  - ⚠️ **API 자동 게시는 아직 꺼져 있습니다** — 개발자 앱이 Pinterest의 trial access 승인 대기 중(2026-08 기준 2개월째)이고, 승인 전에는 App secret이 안 나와 OAuth 자체가 불가능합니다. 그동안은 `content/pins/PINS.md`를 보고 **수동으로** 올리고 `pinterest_publish.py --mark-pinned`로 기록하세요. 자세한 내용과 승인 에스컬레이션 방법은 위 문서에 있습니다.
...
**재검토 요청 타이밍 (중요)**: 지금 바로 "검토 요청"을 누르지 말 것. 같은 상태로 재심사하면
다시 탈락하고 심사 주기(1~2주)만 소모된다. 아래 조건이 모두 갖춰진 뒤 요청할 것:
- [x] 발행 글 25~30편 이상 — **2026-08-31 기준 42편, 충족**
- [ ] 4개 정적 페이지가 라이브 + 내비게이션(페이지 가젯)에 노출
      — 페이지 게시는 완료("Publish Static Pages" 2026-07-19 성공), **페이지 가젯 추가는 미완료**
- [ ] Search Console에서 주요 글이 색인되기 시작함 (실적 탭에 노출 데이터)
- 요청 시: 애드센스 → 사이트 → "문제를 수정했음을 확인합니다" 체크 → 검토 요청
